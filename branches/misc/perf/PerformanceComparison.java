...
/**
 * 
 * このソースをGuiceのSVNのパフォーマンス比較のに上書きしてください。その際に添付のjarをビルドパスに通せば動きます。
 * 
 * 
 * A semi-useless microbenchmark. Spring and Guice constuct the same object
 * graph a bunch of times, and we see who can construct the most per second. As
 * of this writing Guice is more than 50X faster. Also useful for comparing pure
...
public class PerformanceComparison {
	...
	static final Callable<Foo> s3Factory = new Callable<Foo>() {

		private S3Config config = S3.lookup(S3Config.class);
	...
	public static class S3Config {
		...
		@org.seasar3.lookup.Singleton
		public Tee tee() {
			return new TeeImpl(s());
		}

		public Bar bar() {
			return new BarImpl(tee(), i());
		}
	...
	static void iterate(Callable<Foo> callable, String label) throws Exception {
		...
		if (time != 0) {
			System.err.println(label + format.format(count * 1000000000 / time)
					+ " creations/s");
		} else {
